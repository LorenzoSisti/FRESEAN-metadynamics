...
int getInput(char *fnCOM,char *fnTop,char *fnCrd,char *fnVel,char *fnJob,
                ...
                char *fnEigVec,int *freqSel,int *modeStart,int *modeEnd,char *fnOut,
                int needVelo) {
        FILE *io;
        char buffer[300];
        int format;
...
/*PARTIALLY MODULAR*/
        ...
        getInput(fnCOM,fnTop,fnCrd,fnVel,fnJob,
                ...
                fnEigVec,&freqSel,&modeStart,&modeEnd,fnOut,
                needVelo);
/*PARTIALLY MODULAR*/
...
/*MODULAR*/
        ...
        if(nDOF<modeEnd) {
                printf("not enough degrees of freedom: nDOF %d modeEnd %d\n",nDOF,modeEnd);
                exit(1);
        }
        ...
        if(n1!=n2 || n1!=nDOF) {
                printf("incompatible dimensions in %s: nDOF %d nCorr %d n1 %d n2 %d\n",fnEigVec,nDOF,nCorr,n1,n2);
                exit(1);
        }
        for(i=1;i<freqSel;i++) {
                fread(&bc1,sizeof(int),1,io);
...
/*MODULAR*/
        ...
        sprintf(fnOut2,"proj_freq%d_mode%d-%d_%s.dat",freqSel,modeStart,modeEnd,fnOut);
