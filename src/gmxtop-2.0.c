	...
	for(i=0;i<nITP;i++) {
		...
		for(l=k;l<k+itpInfo[i].nRes;l++) {
			...
			while(itpInfo[i].atoms[n].resNr==itpInfo[i].atoms[start-1].resNr && n<itpInfo[i].nAtoms) {
				/*write atom info*/
				m=0;
				while(strcmp(itpInfo[i].atoms[n].atomType,types[m])!=0 && m<nTypes) m++;
				if(m==nTypes) {
					sprintf(buffer,"OOPS: could not find atomType %s in list of atom types obtained from ff\n",itpInfo[i].atoms[n].atomType);
					fatal(buffer);
				}
				fprintf(OUT,"%s\t%.4f\t%.4f\t0.0 %10.5e %10.5e\n",
