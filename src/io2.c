...
int readXST(FILE *in,t_box *box)
{
	...
	sscanf(buffer,"%d %f %f %f %f %f %f %f %f %f",&step,&ax,&ay,&az,&bx,&by,&bz,&cx,&cy,&cz);
	box[0].a.x=ax;
	box[0].a.y=ay;
	box[0].a.z=az;
	box[0].b.x=bx;
...
int prepTRJinput(char *fnCrd,char *fnVel,FILE **cin,FILE **vin,XDR *xdrin,int *format,int *veloFreq,real *time,real *dt,int *nFrames,int needVelo,int nAtoms,int argc)
{
        ...
        {
                if(needVelo==1 && argc==4) fatal("VELOCITIES NEEDED BUT NO INPUT FILE SUPPLIED!\n -> prepTRJinput\n -> io.c\n");
        }
        openInput(format[0],needVelo,cin,vin,xdrin,fnCrd,fnVel);

        if(format[0]==5)
        {
		printf("WARNING: this version of readDCDcrd() ignores box information\n");
		printf("         in the DCD file\n");
		printf("         your progrm needs to call readXST()\n");
