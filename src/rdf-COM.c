...
int doRDF(t_mol *mols,int nMols,t_grp *grps,t_rdf *rdf,t_box box)
{
        real tmp,inc;
        int i,j;
        int put;
        ...
        rdf[0].averDens=((real)grps[rdf[0].grp2Idx].nMols)/tmp;
        inc=1.0/rdf[0].averDens;

        for(i=0;i<grps[rdf[0].grp1Idx].nMols;i++)
        {
                for(j=0;j<grps[rdf[0].grp2Idx].nMols;j++)
                {
                        distPBC(mols[grps[rdf[0].grp1Idx].mols[i]].COM,mols[grps[rdf[0].grp2Idx].mols[j]].COM,box,&tmp);
