...
int main(int argc,char *argv[])
{
	...
	char buffer[300];
	char excl[100];
        char nbAtType[10000][6];
        float nbEpsilon[10000];
        float nbRminHalf[10000];
        int nbCnt=0;
        float nbDummy;
	int found;

        if(argc!=4) fatal("WRONG COMMAND LINE ARGUMENTS:\nNEEDED: input.psf   output.top   prm-file\n -> main\n -> psf.c\n");
        if(strcmp(argv[1],argv[2])==0) fatal("COWARDLY REFUSING TO OVERWRITE INPUT FILE\n -> main\n -> psf.c\n");

        saveOpenRead(&io,argv[3]);
        fgets(buffer,300,io);
        while(strncmp(buffer,"NONBONDED",9)==0) {
                fgets(buffer,300,io);
        }
        fgets(buffer,300,io);
        fgets(buffer,300,io);
        while(strncmp(buffer,"HBOND",5)!=0) {
                i=sscanf(buffer,"%s",excl);
                if(i!=0 && strncmp(excl,"!",1)!=0 && strncmp(excl,"*",1)!=0) {
                        sscanf(buffer,"%s %f %f %f",nbAtType[nbCnt],&nbDummy,&nbEpsilon[nbCnt],&nbRminHalf[nbCnt]);
                        nbCnt++;
                }
		fgets(buffer,300,io);
        }
        fclose(io);
	printf("found %d atom types in %s\n",nbCnt,argv[3]);
	fflush(stdout);
	...
	for(i=0;i<nAtoms;i++) {
		found=0;
		for(j=0;j<nbCnt;j++) {
			if(strcmp(atoms[i].atomType,nbAtType[j])==0) {
				atoms[i].sigma=nbRminHalf[j]*2.0/1.122462;
				atoms[i].epsilon=-4.184*nbEpsilon[j];
				found=1;
				/*printf("found nb parameters for %s: sigma=%f epsilon=%f\n",atoms[i].atomType,atoms[i].sigma,atoms[i].epsilon); fflush(stdout);*/
			}
		}
		if(found==0) {
			sprintf(buffer,"UNKNOWN ATOM TYPE %s NOT FOUND IN PRM-FILE %s\n",atoms[i].atomType,argv[3]);
			fatal(buffer);
		}
	}
	...
	b=0;
        for(i=0;i<=nAtoms;i++)
        {
			...
			e=i-1;
			fprintf(io,"[atoms]\t\t%d\n",e-b+1);
			for(j=b;j<i;j++) fprintf(io,"%-7s %8.4f   %8.4f   %8.4f%12.5e%12.5e\n",atoms[j].atomName,atoms[j].mass,atoms[j].charge,0.0,atoms[j].sigma,atoms[j].epsilon);
			fprintf(io,"[sites]\t\t0\n");
